...
var app = {
		...
		loadApiData:async function(){
				...
				try {
					$.blockUI();   
				  // var url = "http://localhost/iots/index.php?option=com_omg&view=omg&task=device&format=raw&callback=";
					// var url = "http://www.quickiots.com/database/omg_os.json?callback=";

					// var url = api_host+"/database/db.html";
					// var url = "http://localhost/quickiots/website/database/db.html";
					console.log(api_host);

					os = await getJSON(api_host+"/database/omg_os.json");
					cat = await getJSON(api_host+"/database/omg_cat.json");
					config = await getJSON(api_host+"/database/omg_config.json");
					device = await getJSON(api_host+"/database/omg_devices.json");
					device_pin = await getJSON(api_host+"/database/omg_device_pin.json");
					field = await getJSON(api_host+"/database/omg_field.json");
					modules = await getJSON(api_host+"/database/omg_modules.json");
					
					apiData.os = os.RECORDS
					apiData.cat = cat.RECORDS
					apiData.config = config.RECORDS
					apiData.device = device.RECORDS
					apiData.device_pin = device_pin.RECORDS
					apiData.field = field.RECORDS
					apiData.modules = modules.RECORDS
	...
	loadFile:function(fileName){
		...
		drawFields:function(template,container,module_code){
				...
				catInTemplate.forEach(function(cat) {
						...
						(async () => {
								...
								let queryf = 'select * from ? where cat = "'+cat.code+'" and ((  device = ""  or device like "%'+currentDevice.code+'%" ) and exclude_device not like "%'+currentDevice.code+'%" )';
								var fields_in_cat = alasql(queryf,[fields]);
		...
		drawModule:function(data){
			...
			modules = alasql('SELECT * FROM ? WHERE status = 1  order by type ',[modules]);			

			$("#tbl_module tbody" ).html(_.template($("#tpl-module").html(),{rows:modules})); //Draw modules list		
...
async function getJSON(url){
	...
	var ketqua = await new  Promise((resolve, reject) => {
	...
	ketqua = JSON.stringify(ketqua).replace(/null+/g, '""');
